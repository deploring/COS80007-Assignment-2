package au.edu.swin.ajass.config;
...
/**
 * A simple configuration system that is used by
 * both the client and server to retrieve and use
 * settings that need to be defined by the user.
 *
...
public final class Configuration {
    ...
    public Configuration(String fileName, String... requiredKeys) {
        ...
        try {
            ...
            new BufferedReader(new InputStreamReader(getConfigStream(fileName), "UTF-8")).lines().forEach(this::parseLine);
        } catch (IOException | IllegalArgumentException e) {
            e.printStackTrace();
            // Print the error because it is not something that should happen in normal usage.
        }

        // Validate the configuration after.
     ...
     * use the default configuration resource file and copy it outside
     * the JAR for which it will use next time.
     *
    ...
    private InputStream getConfigStream(String fileName) throws IOException {
        ...
        else throw new IllegalStateException("Config file could not be found or loaded");
    }
